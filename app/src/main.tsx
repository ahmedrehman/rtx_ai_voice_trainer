...
import { VoiceAgentV3AppPage } from "./voice_agent_v3/AppV3Page";
import { APP_V3_DEBUG_PAGE } from "./voice_agent_v3/debug";
...
type AppSignal = { type: "idle" | "improvement" | "mistake"; text: string };
...
const pages: Page[] = [
  ...
  { id: "APP_V3_WEBRTC", title: "App V3 WebRTC", module: "App", role: "voice trainer chat using the persistent WebRTC strategy", ready: true, inputs: [], actions: [], output: [], icon: Volume2 },
  ...
  { ...APP_V3_DEBUG_PAGE, icon: iconForPage(APP_V3_DEBUG_PAGE) },
...
function iconForPage(page: DebugPageDefinition) {
  ...
  if (page.module === "app_v3_test") return Volume2;
...
function App() {
  ...
  const [voiceAgentV3Settings, setVoiceAgentV3Settings] = useState<VoiceAgentV2Settings>(VOICE_AGENT_V2_DEFAULT_SETTINGS);
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        <PageView
          ...
          voiceAgentV3Settings={voiceAgentV3Settings}
          setVoiceAgentV3Settings={setVoiceAgentV3Settings}
        />
      </section>
...
function PageView({
  ...
  voiceAgentV3Settings,
  setVoiceAgentV3Settings
}: {
  page: Page;
  voiceAgentSettings: VoiceAgentSettings;
  ...
  voiceAgentV3Settings: VoiceAgentV2Settings;
  setVoiceAgentV3Settings: React.Dispatch<React.SetStateAction<VoiceAgentV2Settings>>;
}) {
  const Icon = page.icon;
  if (page.id === "APP_CHAT") {
    return (
  ...
  if (page.id === "APP_V3_WEBRTC") {
    return (
      <article className="page app-page">
        <VoiceAgentV3AppPage settings={voiceAgentV3Settings} setSettings={setVoiceAgentV3Settings} />
      </article>
    );
  }
  return (
    <article className="page">
      ...
      <h1>{page.title}</h1>
      <p>{page.role}</p>
      {page.module === "voice_agent_frontend_test" && (
        <VoiceAgentFrontendTestPage
          pageId={page.id}
      ...
      {page.module === "app_v3_test" && <VoiceAgentV3AppPage debug settings={voiceAgentV3Settings} setSettings={setVoiceAgentV3Settings} />}
...
function AppVoiceExperience({
  ...
  const [lastSignal, setLastSignal] = useState<AppSignal>({ type: "idle", text: "" });
  ...
  useEffect(() => {
    ...
    if (lastSignal.type !== "idle" && lastCorrectionText) {
  ...
  async function sendTextChat() {
    ...
    try {
      ...
      if (result.status.ok && resultJson?.flags.has_corrections) {
        ...
        setLastSignal(signalFromCorrectionType(resultJson.flags.correction_type));
      } else if (result.status.ok) {
        setLastCorrectionText("");
        setLastSignal({ type: "idle", text: "" });
      } else {
        setLastSignal({ type: "idle", text: "" });
        console.error(result.status.error || "Text chat failed");
      }
      if (speakEnabledRef.current && result.audio) {
        ...
        if (!playResult.played && !playResult.stopped) console.error("Audio returned, but browser playback failed");
      }
      if (debug && id) {
        updateStack(id, {
  ...
  async function analyseAudio(audio: Blob, source: string, options: { listenRunId?: number } = {}) {
    ...
    try {
      ...
      if (result.status.ok && resultJson?.flags.has_corrections) {
        ...
        setLastSignal({ type: "idle", text: "" });
        console.error(result.status.error || "Audio analysis failed");
      }
      let played = false;
      if (speakEnabledRef.current && result.audio) {
        const playResult = await playVoiceAgentAudio(result.audio);
...
function signalFromCorrectionType(correctionType: string): AppSignal {
  if (correctionType === "pronunciation" || correctionType === "accent") {
    return { type: "improvement", text: `Improvement: ${correctionType}` };
  }
  return { type: "mistake", text: `Mistake: ${correctionType || "correction"}` };
}

...
export function createAudioAnalyserDefaultPrompts(options: AudioAnalyserPromptOptions = {}): AudioAnalyserPromptDefaults {
  ...
  return {
    ...
    task: [
      ...
      `Topic/context: ${topic}.`,
      "CONTROL KEYWORDS - DETECTION ONLY, NOT LESSON CONTENT:",
      `ON control phrase: ${keywordOn}.`,
      `OFF control phrase: ${keywordOff}.`,
      "If the user says/writes a control phrase, set only the keyword flags.",
      "Control phrases are not target-language practice text.",
      "Control phrases must not be translated, corrected, answered about, or mentioned in chat_text_to_user.",
      "Ignore the control phrase when checking the remaining user speech for language corrections.",
      "If the user only said/wrote a control phrase, return empty chat_text_to_user, empty text_corrected, empty hint, has_corrections false, correction_type none, and is_chat_answer_or_correction none.",
      "Detect keyword_on_sent only when the user actually says or writes the exact ON control phrase.",
      "Detect keyword_off_sent only when the user actually says or writes the exact OFF control phrase.",
      "If neither exact control phrase is present, both keyword flags must be false.",
